...
function pixpine_paypal_order_success()
{
    global $wpdb;
    if (is_user_logged_in()) {
    $user_id = get_current_user_id();
    $msg = '';
      ...
      $discount = get_user_specific_discount();
      $html = order_confirmation_email($product_ids, 'paypal', $discount, $order_id);
      $email = 'orders@pixpine.site, innovawebdeveloper@gmail.com, harun@encoderit.net, harun.encoderit@gmail.com, '.$current_user->user_email;
      pixpine_send_html_email($email, ' Thank you for your payment', $html);
      echo site_url() . '/downloads?type=new-purchase';
    }
    exit;
}
add_action('wp_ajax_pixpine_paypal_order_success', 'pixpine_paypal_order_success');
// add_action('wp_ajax_nopriv_pixpine_paypal_order_success', 'pixpine_paypal_order_success');